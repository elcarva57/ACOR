...
//----------------------------------------------------------------------
 ...
 //lectura de nuevos valores
             ...
             grafico24h[1439].LeturasAnemT2 = (SAnemometroT2)/CuentaMensMeteo;
               if  (grafico24h[1439].LeturasAnemT2 > 255 ) grafico24h[1439].LeturasAnemT2 = 0;
         ...
         //     Form1->Memo1->Lines->Add("AnemometroT 60sg : " + AnsiString(SAnemometroT) +" Div: " + AnsiString(Form1->Timer2->Interval/Form1->Timer3->Interval));
             if(grafico24h[1439].LeturasAnemT2 > 10)  // solo cambia veleta si velocidad apreciable
 ...
 //     if(CBimagen->ItemIndex != 3)
  ...
  //    if(FotoActual > nfotos)
    //    Beep();
      ...
      int fwh =FotoPrincipal->fullhalfwide();       //clave como se ha terminado de recibir la foto se llama a la funcion
  //    Form1->Mhistory->Lines->Add("fhw =" + AnsiString(fwh));  //y se presenta el resultado
      if (A_FinFoto == true)
      {
        A_FinFoto = false;
...
//  ACOR_SetImage((short*)Form2->Foto->Pixeles, Form2->Foto->BM->Width, Form2->Foto->BM->Height);
  AF_SetImage((short*)Form2->Foto->Pixeles, ancho, alto);
  AF_SetSkyRegion(0, 0, ancho-1, alto-1);
  AF_SetCentroidRegion(0, 0, ancho-1, alto-1);
...
// AF_SetImage(imagen, width, height);
 ...
 void __fastcall TForm1::BCalcFondoClick(TObject *Sender)
{
  AF_SetSkyRegion(X1F, Y1F, X2F, Y2F);
  Memo1->Lines->Add("Rec. Fondo       X1F =" + AnsiString(X1F) + "  X2F =" + AnsiString(X2F) + "  Y1F =" + AnsiString(Y1F) + "  Y2F =" + AnsiString(Y2F));
...
void __fastcall TForm1::BEstrellaClick(TObject *Sender)
{
AF_SetCentroidRegion(X1F, Y1F, X2F, Y2F);
 Memo1->Lines->Add("Rect. Estrella:  X1F =" + AnsiString(X1F) + "  X2F =" + AnsiString(X2F) + "Y1F =" + AnsiString(Y1F) + "  Y2F =" + AnsiString(Y2F));
}
//---------------------------------------------------------------------------

void __fastcall TForm1::CalculaHFDClick(TObject *Sender)
{
//ACOR_SetImage(Pixeles,24,24);
// Calcula_Imagen((short*)FotoPrincipal->Pixeles, FotoPrincipal->BM->Width, FotoPrincipal->BM->Height, PContador->Caption.ToInt());
}
//---------------------------------------------------------------------------
