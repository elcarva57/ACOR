...
double DiaJuliano(int anno, int mes, int dia, int hora, int min, int seg ){
  double JUL = 2400000.0 - 678987.0 + 367*anno +
  275*mes/9.0 + dia - 7*(anno + (mes + 9)/12.0)/4.0 + (hora * 3600 + min * 60 + seg)/86400.0 ;
  return (JUL);
}
...
//  TDateTime *J2000 = new TDateTime(2000,1,1);
  ...
  diaj = Fe.CurrentDateTime() +0.5 - JD;
//  diaj = diaj - JD;
  return (diaj);
}
...
double CalAzimut(double ALT, double ARec, double DEC) {
  ...
  hside = CalHSLgrados();
  HA = hside -  ARec;
//Historico->Mhistory->Lines->Add("HSid: " + AnsiString(hside) + " ARec: " + AnsiString(ARec) );
...
{
        AnsiString startcab = "<" + cab + ">";
        AnsiString endcab = "</" + cab + ">";
        int ind1 = 0, ind2 = 0;

        ind1 = buf.Pos( startcab );
        if(ind1 == 0)
        {
                *dato = "";
                return -1;
        ...
        ind2 = buf.Pos( endcab );
        *dato = buf.SubString( ind1 + startcab.Length(), ind2 - (ind1 + startcab.Length()) ).Trim();
...
//---------------------------------------------------------------------------
void ProcesarPicture(AnsiString s)
{
AnsiString s2, s3;

        nPicture++;
        if (BuscarXML("Filter", s, &s2, &s3) == 0)
           Destino[nTarget].Foto[nPicture].Filter = s2.ToInt();
        if (BuscarXML("Count", s, &s2, &s3) ==0)
           Destino[nTarget].Foto[nPicture].Count = s2.ToInt();
        if (BuscarXML("Binning", s, &s2, &s3)== 0)
          Destino[nTarget].Foto[nPicture].Bining = s2.ToInt();
        if (BuscarXML("ExposureTime", s, &s2, &s3)==0)
          Destino[nTarget].Foto[nPicture].ExposureTime = s2.ToInt();
}



//---------------------------------------------------------------------------
void ProcesarTarget(AnsiString s)
{
   AnsiString s2, s1, sf;
   TDateTime f;
   s1 = s;
   ...
   BuscarXML("Name", s, &s2, &s1);
   strcpy(Destino[nTarget].Name, s2.c_str());
   BuscarXML("ID", s, &s2, &s1);
   strcpy(Destino[nTarget].ID, s2.c_str());

   if (BuscarXML("RightAscension", s, &s2, &s1) == 0) {
     y = strlen(s2.c_str());
     for (x= 0; x < y; x++){
       if(s2.c_str()[x]== '.') s2.c_str()[x] = ',';
     }
     Destino[nTarget].RightAscension = s2.ToDouble();  //pasa ar a horas para LX
     }
     else
       Destino[nTarget].RightAscension = -1;
   if (BuscarXML("Declination", s, &s2, &s1) == 0) {
     y = strlen(s2.c_str());
     for (x= 0; x < y; x++){
       if(s2.c_str()[x]== '.') s2.c_str()[x] = ',';
     }
     Destino[nTarget].Declination = s2.ToDouble();
   }
     else
       Destino[nTarget].Declination = -101;

   BuscarXML("DateTime", s, &s2, &s1);
   if(GenerarFecha(s2, &f) == 0)
        Destino[nTarget].DateTime =  f;
   else
        Fprog->Mver_rtml->Lines->Add("Error Fecha" );

   nPicture = 0;
   while(BuscarXML("Picture", s1, &s2, &s1) != -1)
   {
     ProcesarPicture(s2);
   }
...
void ProcesarFichero(AnsiString s)
{
   AnsiString s2, s1;
   s1 = s;
   while(BuscarXML("Target", s1, &s2, &s1) != -1)
   {
        ProcesarTarget(s2);
   }
}
...
TDateTime Fecha1;
  ...
  Fecha1 = Fecha.CurrentTime();
    if(AutoRun != Stop ) {
      ...
      Historico->Mhistory->Lines->Add(AnsiString(Fecha1) +"  PAUSA Manual" );
    }
    else{
      if ( AutoRun == Stop) {
        BCarga->Font->Size = 8;
        BCarga->Caption = "Pausar";
        AutoRun = estado;
        Historico->Mhistory->Lines->Add(AnsiString(Fecha1) +"  REINICIO Manual" );
      }
    }
  }
}
...
{
  AutoRun = 0;
  Historico->Mhistory->Lines->Add("STOP Manual Adquisicion Automatica" );
...
TDateTime Fecha1;
  ...
  if (AutoRun == HayTarget) {
     ...
     Historico->Mhistory->Lines->Add(" ");
     Historico->Mhistory->Lines->Add(" ------------------------------------ ");
     Historico->Mhistory->Lines->Add(ODxml->FileName);
     Historico->Mhistory->Lines->Add(AnsiString(Fecha1) + " Lanzado Fichero con "
                                                        + AnsiString(nTarget) + "  Objetos");
    ...
    if (Repesca == 0) {
           if (cont_dest < destinos) {
             ...
             Historico->Mhistory->Lines->Add(" " );
             Historico->Mhistory->Lines->Add("Esperando Hora " +  Destino[cont_dest].DateTime );
...
//                 AutoRun = EsperaHorizonte;
                 Historico->Mhistory->Lines->Add("Esperando Horizonte " + AnsiString(esp_horizon) + " destinos. " );
                 Historico->Mhistory->Lines->Add(" ");
                 esp_horizon = 0;
                 cont_dest = 0;
               }
                 ...
                 Fecha1 = Fecha.CurrentDateTime();
                 Historico->Mhistory->Lines->Add(" " );
                 Historico->Mhistory->Lines->Add(AnsiString(Fecha1) + "   Fin del Script.");
                 Historico->Mhistory->Lines->Add(" " );
...
//          Historico->Mhistory->Lines->Add("Espero Horizonte Destino " +  AnsiString(cont_dest+1));
...
//      LRestante->Caption = ( "Destino " + AnsiString(cont_dest+1) +"  ALT: " + AnsiString(cad1) + "   AZ: " +AnsiString(cad2));
      if ( alt > EAlturaInf->Text && alt < EAlturaSup->Text  && azi > EAzimut1->Text && azi < EAzimut2->Text )
      {
         ...
         Historico->Mhistory->Lines->Add("Horizonte Destino " + AnsiString( cont_dest +1) + " Alcanzado.");
         Historico->Mhistory->Lines->Add("Colocando Filtro " + AnsiString( Destino[cont_dest].Foto[cont_series+1].Filter) );
         ...
         Historico->Mhistory->Lines->Add("Moviendo Telescopio  AR: "
                                      + AnsiString(cad2) + "h  Dec: "  + AnsiString(cad1) + "º");
    ...
    if (Fecha.CurrentDateTime() >= Destino[cont_dest].DateTime )
    {
      ...
      if ( alt > EAlturaInf->Text && alt < EAlturaSup->Text  && azi > EAzimut1->Text && azi < EAzimut2->Text )
      {
         ...
         sprintf(cad2,"%003.0004f", Destino[cont_dest].RightAscension/15);
         Filtro->mover_rueda( Destino[cont_dest].Foto[cont_series+1].Filter);
         Historico->Mhistory->Lines->Add("Colocando Filtro " +
...
//        if (cont_dest < destinos) {
          ...
          Historico->Mhistory->Lines->Add("Alt " +  AnsiString(alt) + "   Azi " + AnsiString(azi));
          cont_dest++;
          esp_horizon++;
          Historico->Mhistory->Lines->Add("Destino " + AnsiString(cont_dest) + "  Bajo el Horizonte. " );
          Historico->Mhistory->Lines->Add("Retrasados " + AnsiString(esp_horizon) + "  destinos. " );
      }
    }
  }
       ...
       if ( (abs(dde) < 5) && (abs(dar) < 3) ) {
         ...
         sprintf(cad1,"%003.0004f",(tDe - Destino[cont_dest].Declination));
         sprintf(cad2,"%003.0004f",(tRa - Destino[cont_dest].RightAscension));
         Historico->Mhistory->Lines->Add("Coordenadas Alcanzadas: dif DEC " + AnsiString(cad1)
      ...
      if ( DogEsperaStopLX > 20) {
         ...
         AutoRun = HacerSeries;
         Historico->Mhistory->Lines->Add(" Coordenadas No Alcanzadas " );
      }
  }
     ...
     if (cont_series < Destino[cont_dest].Series){
         AutoRun = InicioSerie;
         Fecha1 = Fecha.CurrentTime();
         Historico->Mhistory->Lines->Add( AnsiString(Fecha1) + "  Inicio Serie" );
         cont_series++;
     }
         ...
         if( DogEsperaStopLX > 20) {
           ...
           Historico->Mhistory->Lines->Add( AnsiString(Fecha1) + "  Destino " + AnsiString(cont_dest + 1)
                                                                            + " Atrasado");
           esp_horizon++;
         }
         else {
